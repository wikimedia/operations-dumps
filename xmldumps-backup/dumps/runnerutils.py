...
class Checksummer(object):
    ...
    def checksum(self, file_obj, runner):
        ...
        if self._enabled:
            ...
            dumpfile = DumpFile(self.wiki, runner.dump_dir.filenamePublicPath(file_obj), None, self.verbose)
    ...
    def _get_checksum_filename(self):
        ...
        return self.dump_dir.filenamePublicPath(file_obj)
...
class Status(object):
    ...
    def report_file(self, file_obj, item_status):
        filename = self.dump_dir.filenamePublicPath(file_obj)
        if exists(filename):
...
class SymLinks(object):
    ...
    def save_symlink(self, dumpfile):
        if self._enabled:
            ...
            realfile = self.dump_dir.filenamePublicPath(dumpfile)
...
class Feeds(object):
    ...
    def save_feed(self, file_obj):
        if self._enabled:
            self.make_dir(self.dump_dir.latest_dir())
            filename_and_path = self.dump_dir.webPath(file_obj)
            ...
            rss_path = os.path.join(self.dump_dir.latest_dir(), self.dbName + "-latest-" + file_obj.basename + "-rss.xml")
