...
class TestFileLister(BaseDumpsTestCase):
    ...
    def test_xmlstubs_lister(self, _mock_get_some_stats, _mock_get_known_tables):
        ...
        building = self.dfsort(stubs_job.oflister.list_outfiles_for_build_command(
            stubs_job.oflister.makeargs(self.en['dump_dir'])))
        publish = self.dfsort(stubs_job.oflister.list_outfiles_to_publish(
            stubs_job.oflister.makeargs(self.en['dump_dir'])))
        ...
        cleanup_inprog = self.dfsort(stubs_job.oflister.list_inprog_files_for_cleanup(
            stubs_job.oflister.makeargs(self.en['dump_dir'])))
        cleanup = self.dfsort(stubs_job.oflister.list_outfiles_for_cleanup(
            stubs_job.oflister.makeargs(self.en['dump_dir'])))
        for_input = self.dfsort(stubs_job.oflister.list_outfiles_for_input(
            stubs_job.oflister.makeargs(self.en['dump_dir'])))
        ...
        truncated = self.dfsort(stubs_job.oflister.list_truncated_empty_outfiles(
            stubs_job.oflister.makeargs(self.en['dump_dir'])))
        ...
        cleanup_inprog = stubs_job.oflister.list_inprog_files_for_cleanup(
            stubs_job.oflister.makeargs(self.en['dump_dir']))
        cleanup = stubs_job.oflister.list_outfiles_for_cleanup(
            stubs_job.oflister.makeargs(self.en['dump_dir']))
        for_input = stubs_job.oflister.list_outfiles_for_input(
            stubs_job.oflister.makeargs(self.en['dump_dir']))
        ...
        truncated = stubs_job.oflister.list_truncated_empty_outfiles(
            stubs_job.oflister.makeargs(self.en['dump_dir']))
    ...
    def test_xmlcontent_lister(self):
        ...
        building = self.dfsort(articles_job.oflister.list_outfiles_for_build_command(
            articles_job.oflister.makeargs(self.en['dump_dir'])))
        publish = self.dfsort(articles_job.oflister.list_outfiles_to_publish(
            articles_job.oflister.makeargs(self.en['dump_dir'])))
        ...
        cleanup_inprog = self.dfsort(articles_job.oflister.list_inprog_files_for_cleanup(
            articles_job.oflister.makeargs(self.en['dump_dir'])))
        ...
        cleanup = self.dfsort(articles_job.oflister.list_outfiles_for_cleanup(
            articles_job.oflister.makeargs(self.en['dump_dir'])))
        some_parts_articles = [
        ...
        for_input = self.dfsort(articles_job.oflister.list_outfiles_for_input(
            articles_job.oflister.makeargs(self.en['dump_dir'])))
        with self.subTest('list article outputs for input'):
            self.assertEqual(for_input, some_parts_articles_dfnames)

        truncated = self.dfsort(articles_job.oflister.list_truncated_empty_outfiles(
            articles_job.oflister.makeargs(self.en['dump_dir'])))
